...
!     Last change: SOU 2/13/2014 1:34:27 PM
!     ******************************************************************
      SUBROUTINE LNDCLC
!-----------------------------------------------------------------------
!     SUBROUTINE LNDCLC
...
REAL (KIND=8):: DEM13,DEM12,DEM11, DIFF, DIFFBEF,FlowNeg
REAL (KIND=8):: OtterBalance,Stream,DiffNeg,DiffNegBef
INTEGER (KIND=4) :: JJJ
!-----------------------------------------------------------------------
!     SAMPLE CODE FOR GENERAL MODEL
!-----------------------------------------------------------------------
IF (J.EQ.3.AND.K.EQ.9) THEN
  JJJ=1
END IF
QX(15,J,K)=0.0
OtterBalance=RSBAK(1)
QX(18,J,K)=Min(0.0,OtterBalance)
DEM13=WETDMD(1)
DEM12=DMDMD(1)
DEM11=ARDEM(1)
QX(1,J,K)=Max(0.0,OtterBalance+DEM13+DEM12+DEM11)
CALL ARSUB(1)
QX(8,J,K)=QX(6,J,K)+QX(5,J,K)
CALL MUNSUB(1)
QX(13,J,K)=QX(11,J,K)+QX(10,J,K)
CALL WETLND(1)
QX(15,J,K)=QX(13,J,K)-QX(14,J,K)
Stream=MAX(0.0,MIN(QX(15,J,K),QX(1,J,K)))
DIFF=OtterBalance-QX(15,J,K)-QX(18,J,K)
DiffNeg=Min(0.0,Stream+DIFF)
QX(18,J,K)=QX(18,J,K)+DiffNeg
DIFF=DIFF-DiffNeg
IF (ABS(DIFF).GT.0.1.OR.ABS(DiffNeg).GT.0.1) THEN
  DO
    CALL MUNBAK(1)
    CALL ARBAK(1)
    ...
    QX(5,J,K)=0.0
    CALL ARSUB(1)
    QX(8,J,K)=QX(5,J,K)+QX(6,J,K)
    CALL MUNSUB(1)
    QX(13,J,K)=QX(10,J,K)+QX(11,J,K)
    CALL WETLND(1)
    QX(15,J,K)=QX(13,J,K)-QX(14,J,K)
    DIFFBEF=DIFF
    DiffNegBef=DiffNeg
    ...
    QX(18,J,K)=QX(18,J,K)+DIFFNEG
    DIFF=DIFF-DiffNeg
    !IF ((ABS(DIFF).LE.0.1.AND.ABS(DiffNeg).LE.0.1).OR. &
    !  (ABS(DIFF-DIFFBEF).LE.0.1.AND.ABS(DiffNeg-DiffNegBef).LE.0.1)) THEN
    IF ((ABS(DIFF).LE.0.1.AND.ABS(DiffNeg).LE.0.1)) THEN
      EXIT
    END IF
  END DO
END IF
